	 ...
	 //form data
      ...
      <div class="page-heading"> 
        <h2 class="iod">Update product <ul class="list-inline"><li><a class="btn btn-primary btn-sm" href="<?php echo base_url().'admin/product'; ?>">&laquo; Back</a></li><li><button type="submit" class="btn btn-primary btn-sm">Save</button></li><li><button type="submit" class="btn btn-primary btn-sm">Save & Continue</button></li></h2>
      ...
      ?>
		...
		<div class="form-group col-sm-12">
          ...
          <label  class="control-label col-sm-3">Weight / size<small style="color:red">*</small></label>
             <div class="col-sm-9">  <input type="text" class="form-control" <?php if($mcat[0]['business_type']==31) {echo "";}else {echo "required";}?> name="weight" value="<?php if($this->input->post('weight')!='') { echo $this->input->post('weight'); } else { echo $prod['weight']; }  ?>" >
          </div>
          </div>
...
<div class="col-sm-12">	 
          ...
          <label  class="control-label col-sm-5">Actual Price</label>
		   ...
		   <div class="form-group col-lg-6 col-mg-6 col-sm-6">
            <label  class="control-label col-sm-5">Special Price</label>
             <div class="col-sm-7"> <input type="number" class="form-control"  name="p_d_price" value="<?php if($this->input->post('p_d_price')!='') { echo $this->input->post('p_d_price'); } else { echo $prod['p_d_price']; }  ?>" >
          </div> 
          </div> 
  ...
  <div class="form-group col-lg-6 col-mg-6 col-sm-6">
		...
		<input type="hidden" value="<?php echo $prod['image'];?>" name="image_old">
               ...
               <div class="col-sm-7"><?php if($prod['image']!='') { echo '<img width="150" class="img-responsive" src="'.base_url().'../main-admin/images/product/'.$prod['image'].'" >'; } ?> </div>
         ...
         <?php if($prod['images']!=''){
				...
				foreach($imagesArray as $imagesVal) {
					echo '<div class="remove-i-div-'.$count.'"><div class="form-group col-sm-3"><input type="hidden" value="'.$imagesVal.'" name="images_old[]"><img class="img-responsive" src="'.base_url().'../main-admin/images/product/'.$imagesVal.'" ><button data-img="'.$imagesVal.'" data=".remove-i-div-'.$count.'" type="button" class="remove-btn glyphicon glyphicon-remove remove-image-div"></button></div></div>';
					$count++;
				}
			} ?>	
...
<p class="clr"> <button class="btn btn-success btn-sm add-upload-img" type="button">Add Image</button><br> </p> 			
	...
	<div class="panel panel-default">
      ...
      <?php 
	  } /******* end if not merchant product ********/
	  else { echo '<p>No permission for edit this product.</p>'; } 
	  echo form_close(); ?>
