...
foreach($redeam as $con){ 
	...
	echo '<tr><td>'.$i.'</td><td><a href="'.base_url().'admin/redeam/edit/'.$con['rd_id'].'">'.$con['f_name'].'</a></td><td><a href="'.base_url().'admin/redeam/edit/'.$con['rd_id'].'">'.$con['email'].'</a></td><td>'.$con['redeem'].'</td><td>'.$con['after_tds'].'</td><td>'.$con['redeem_status'].'</td><td>'.$con['var_status'].'</td><td>'.$con['my_bliss_req'].'</td>';
echo '</tr>';
$i++;
}
?>
