...
foreach($sale as $con){  
	...
	$tax = ($con['gtotal']*12)/100;
	$tdss = $tax/2;
	$total_tds += $con['total_gst'];
	$total_amt += $con['gtotal'];
	echo '<tr><td>'.$i.'</td><td>'.$con['f_name'].' '.$con['l_name'].' ('.$con['customer_id'].')</td><td>'.round($con['gtotal'],2).'</td>
	<td>'.$con['total_gst'].'</td></td>
	<td>'.$con['pancard'].'</td><td>'.date('d F Y',strtotime($con['tdate'])).'</td>';
