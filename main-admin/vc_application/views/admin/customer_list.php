 ...
 <?php
	...
	<?php } ?> 
	  <p>&nbsp;</p>
	   ...
	   <thead> <tr><th>ID</th> <th>Name</th><th>Distributor ID</th><th>Sponsor ID</th><th>Phone</th><th>DOJ</th><th>Activation Date</th><th>Wallet</th><!--<th>Repurchase</th> --><th>Status</th><th></th> </tr> </thead> 
<tbody> 
<?php 
$i = 1;
foreach($customer as $con){
	...
	if($con['package_used']=='') { $activate_date = '---'; } else { $activate_date = date('d F Y',strtotime($con['package_used']));  } 
	
	//if($con['tid'] != '' || $con['oid'] != '') { $repurchase = 'yes'; } else { $repurchase = '--'; }
	echo '<tr><td>'.$i.'</td><td><a href="'.base_url().'admin/customer/edit/'.$con['id'].'">'.$con['f_name'].'</a></td><td>'.$con['customer_id'].'</td><td>'.$con['direct_customer_id'].'</td><td>'.$con['phone'].'</td><td>'.date('d F Y',strtotime($con['rdate'])).'</td><td>'.$activate_date.'</td><td>'.$con['bliss_amount'].'</td><td>'.$status.'</td>';
?><td><form method="post" action="<?php echo base_url(); ?>../member-login" target="_blank" class="form form-inline">	  <p> 		  <input type="hidden" class="form-control" required value="<?php echo $con['customer_id']; ?>" name="bcono" style="height:auto;"> 		  <input type="submit" name="submit" class="btn btn-primary" value="Login">	  	  <input type="hidden" name="auth" value="<?php echo md5('@#96pp~~'.date('YmdH').md5('Awr@sw@Y'));?>">	  </p>	</form></td>
