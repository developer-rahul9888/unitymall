...
class Sale extends CI_Controller {
  ...
  public function index() {
	...
	//load the view
	 ...
	 public function send_stock_qty(){
		  
	$data['send_franchise_stock'] = $this->sale_model->get_send_franchise_stock_id(); 
   // echo '<pre>';print_r($data['send_franchise_stock']);die();
    $data['main_content'] = 'admin/send_stock_qty'; 
    $this->load->view('includes/admin/template', $data); 
  ...
  if($data['customer_info'][0]['parent_customer_id']!='') { $data['sponser_name'] = $this->sale_model->get_customer_info($data['customer_info'][0]['parent_customer_id']);  }
   ...
   $this->load->view('includes/admin/template', $data);   
	...
	//$data['customer'] = $this->sale_model->get_customer_by_date($sdate,$edate);
    	 
	$data['sale'] = $this->sale_model->get_all_pin_sale($sdate,$edate);
  ...
  public function daily_weekly_income(){
   ...
   $header = array("Sr.","Used By","Date","PIN","Amount"); 
   fputcsv($file, $header);
   	 $customer = $this->sale_model->daily_weakly_pin($sdate, $edate,$pin);
	
   	if(!empty($customer)) { 
		$i = 1;
       foreach ($customer as $key=>$line){ 
		$amount = $line['p_amount']; 
		$date = date('d F Y',strtotime($line['used_on']));
	     $csv_val = array($i,$line['f_name'].' '.$line['l_name'].'('.$line['customer_id'].')',$date,$line['pinid'],$amount);
         fputcsv($file,$csv_val); 
		 $i++;
       }
       fclose($file); 
          ...
          if ( $parent_customer_id != '' && $repurchase != '' && $sale_one_bv > 0 ) {
            while ( $p < 1 ) {
              ...
              if ( !empty( $parent_user ) ) {
                ...
                //$add_pv_array[] = array( 'order_id' => $order_id, 'amount' => $sale_one_bv, 'user_id' => $parent_user[ 0 ][ 'id' ], 'user_id_send_by' => $cust_id,  'pay_level' => $dis_level ,  'position' => $position);
 ...
 public function send_stock_qty_del(){
  
  $id = $this->uri->segment(5); 
     $return = $this->sale_model->delete_stock_detail($id); 
          $this->session->set_flashdata('delete', 'true'); 
    redirect(base_url().'admin/sale/send_stock_qty');
 }  
}